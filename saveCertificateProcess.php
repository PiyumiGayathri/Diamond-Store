...
if (isset($_FILES["certificate"])) {
    ...
    $certificate_rs = Database::search("SELECT * FROM `certificate` WHERE `user_email`='" . $_SESSION["user"]["email"] . "' AND `status`='0'");
    $certificate_rows = $certificate_rs->num_rows;

    if($certificate_rows == 0){
        Database::iud("INSERT INTO `certificate` (`code`,`status`,`user_email`) VALUES 
        ('" . $_FILES["certificate"]['name'] . "' , '0' ,'" . $_SESSION["user"]["email"] . "')");
    
    echo "Successfully Added!";
    }else{
        echo "Error Occured! This file uploaded before, Choose Another File!";
    }
